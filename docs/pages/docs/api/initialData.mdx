...
## Example payload
An example payload can be found in `src/examples/initialData.ts`.
...
## Validation
The `initialData` object is validated before Isoatlas renders the scene, and an error is thrown if invalid data is detected.
